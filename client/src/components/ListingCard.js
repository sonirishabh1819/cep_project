  ...
  return (
    ...
    >
      <Link href={`/listings/${listing._id}`} className="block">
        <div className="bg-white rounded-2xl overflow-hidden border border-[#e8ddd0] group-hover:border-[#c41e3a]/30 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-[#c41e3a]/10">
          {/* Image */}
          <div className="relative aspect-[4/3] overflow-hidden bg-[#f0e6d9]">
            {listing.images?.length > 0 ? (
              ...
              <div className={`w-full h-full flex items-center justify-center bg-gradient-to-br ${style.from} ${style.to} relative overflow-hidden`}>
            ...
            {/* Save / Heart button */}
            <button
              className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                setSaved((s) => !s);
            ...
            >
              <motion.div
                ...
                className={`w-9 h-9 rounded-full backdrop-blur-sm flex items-center justify-center shadow-lg ${saved ? 'bg-[#c41e3a]' : 'bg-white/90'}`}
              >
                <svg
                  className={`w-4 h-4 transition-colors ${saved ? 'text-white' : 'text-[#c41e3a]'}`}
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-between mb-4">
              <span className={`text-xl font-bold font-display transition-all duration-300 ${listing.isDonation ? 'text-[#c41e3a]' : 'text-[#1a1210] group-hover:text-[#c41e3a]'}`}>
